 ...
 * @param {BrowserWindow|number} win
 */
windowPlus.getWinInfo = function (win) {
  let winID = -1;
...
function _saveWindowStates () {
  ...
  for ( let i = 0; i < _winInfos.length; ++i ) {
    ...
    for (let name in winInfo) {
      winProfile[name] = winInfo[name];
    }

    if ( !winProfile.url ) {
      winProfile.url = win.webContents.getURL();
    }

    profileData.windows.push(winProfile);
